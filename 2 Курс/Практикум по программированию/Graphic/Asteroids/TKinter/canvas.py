...
class Canvas:
    ...
    def __init__(self, master, width, height):
        self._canvas = tkinter.Canvas(
            master, width=width, height=height, bd=2, bg="black", highlightthickness=0
        )
    ...
    def draw_line(self, point1, point2, line_width, line_color):
        self._canvas.create_line(
            ...
            width=line_width,
            fill=map_color(line_color),
        )

    def draw_polyline(self, point_list, line_width, line_color):
        for i in range(1, len(point_list)):
            self.draw_line(
                point_list[i - 1], point_list[i], line_width, map_color(line_color)
            )
    ...
    def draw_circle(
        self, center_point, radius, line_width, line_color, fill_color=None
    ):
        points = [
    ...
    def draw_point(self, point, color):
        self._canvas.create_line(
            [point[0], point[1], point[0] + 1, point[1]], fill=map_color(color)
        )
    ...
    def draw_image(
        ...
        img = image._get_tkimage(
            center_source, width_height_source, width_height_dest, rotation
        )
